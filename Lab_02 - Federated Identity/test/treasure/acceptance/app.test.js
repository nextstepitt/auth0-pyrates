...
describe('homeController', () => {
    ...
    it('Launches automatically from the command line', async () => {
        ...
        process.chdir('./src/treasure')                  // .env is in the pyrates folder; pool is set to 'forks' in vitest.config.js to allow this.
        ...
        const homeController = await import('../../../src/treasure/homeController')
        ...
        expect(homeControllerSpy).toBeCalledWith(process.env.ISSUER_BASE_URL, process.env.CLIENT_ID, process.env.CLIENT_SECRET, process.env.SECRET, process.env.APPLICATION_PORT, process.env.BASE_URL, process.env.PYRATES_URL)
    })
})
